...
function displayProducts(category) {
    ...
    productList.innerHTML = ''; // پاک کردن محصولات قبلی

    const productsToShow = products.filter(product => product.category === category && product.id >= 1 && product.id <= 6);

    productsToShow.forEach(product => {
        const productLink = document.createElement('a');
        productLink.href = `../productidpage/product${product.id}.html`; // لینک متفاوت برای هر محصول
...
// function activateMenuScripts() {
//     const menuItems = document.querySelectorAll('.first_li');
