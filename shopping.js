...
function activateMenuScripts() {
    ...
    // document.querySelectorAll('.first_li').forEach(item => {
    //     item.addEventListener('mouseenter', function() {
    //         const subMenu = item.querySelector('.ul2');
    //         if (subMenu) {
...
function loadCart() {
    ...
    if (cart.length > 0) {
        ...
        cart.forEach(function (item, index) {
            ...
            var priceWithoutCommas = item.price.replace(/,/g, '');
            var itemTotalPrice = parseInt(priceWithoutCommas.replace(' تومان', '')) * item.quantity;
        ...
        totalPriceElement.innerText = totalSum.toLocaleString() + ' تومان';
        totalSum += shippingCost;
        totalSumElement.innerText = totalSum.toLocaleString() + ' تومان';
    } else {
